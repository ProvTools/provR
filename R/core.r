...
.ddg.create.data.use.edges.for.console.cmd <- function(vars.set, cmd, cmd.pos, for.caller) {
    ...
    for (var in vars.used) {
        ...
        if (.ddg.data.node.exists(var, scope)) {
            ...
            # parse(text=cmd.expr), 'appears to use it for procedure node', cmd, '.')
            # .ddg.insert.error.message(error.msg)
        }
    }
}
...
# .ddg.parse.commands takes as input a list of parsed expressions from an R
# script and creates DDG nodes for each command. If environ is an environment, it
# executes the commands in that environment immediately before creating the
# respective nodes for that command, and then creates the data nodes based on the
# information available in the environment. If environ is not NULL, calls to
# ddg.* are not executed so only the clean script is processed.  If
# annotate.inside is TRUE, ddg.function, ddg.eval and ddg.return.value are added
# to each function definition and ddg.eval is added to control statements before
# commands are processed. If save.debug is TRUE, changes to the script are saved
# in the ddg/debug directory.  ddg.annotate.on and ddg.annotate.off may be used
# to limit the functions that are annotated or not annotated, respectively.  If
# run.commands is false, the commands are not executed.  This allows us to build
# ddgs for commands run from the console as those commands have already been
# executed.

# exprs - list of parsed R statements script.name - name of the script the
# statements came from script.num - the number of the script in the sourced
# scripts table environ - environment in which commands should be executed.
# ignore.patterns (optional) - a vector of regular expressions.  Any commands
# matching these expressions will not be parsed (i.e. no nodes will be created
# for them).  node.name (optional) - name for the collapsible node under which
# this DDG should be stored.  run.commands (optional) - commands are executed
# only when environ is an environment and run.commands is TRUE.  echo (optional)
# - print each expression after parsing print.eval (optional) - print result of
# each evaluation.  max.deparse.length (optional) - maximum number of characters
# output for deparse of a single expression.  called.from.ddg.eval(optional) -
# whether called from ddg.eval cmds - list of DDG Statements that correspond to
# the exprs passed in.  This is currently only used when called from ddg.eval.
# Normally, ddg.parse.commands creates the DDG Statement objects.

## .ddg.parse.commands <- function (exprs, script.name='', script.num=NA, environ,
## ignore.patterns=c('^ddg.'), node.name='Console', run.commands = FALSE,
## echo=FALSE, print.eval=echo, max.deparse.length=150,
## called.from.ddg.eval=FALSE, cmds=NULL) {

## return.value <- NULL

## # Gather all the information that we need about the statements if
## (is.null(cmds)) { cmds <- .ddg.create.DDGStatements (exprs, script.name,
## script.num)

## if (.ddg.save.debug()) { .ddg.save.annotated.script(cmds, script.name) } }
## num.cmds <- length(cmds)

## # Figure out if we will execute commands or not.  execute <- run.commands &
## !is.null(environ) & is.environment(environ) inside.func <-
## (.ddg.get('.ddg.func.depth') > 0)

## # Attempt to close the previous collapsible command node if a ddg # exists if
## (.ddg.is.init() && !inside.func) .ddg.close.last.command.node(environ,
## initial=TRUE)

## # Get the last command in the new commands and check to see if # we need to
## create a new .ddg.last.cmd node for future reference.  if (!inside.func) {
## .ddg.last.cmd <- cmds[[num.cmds]] if (.ddg.last.cmd@isDdgFunc) { .ddg.last.cmd
## <- NULL }

## else if (!execute) { cmds <- cmds[1:num.cmds-1] } }

## # Create start and end nodes to allow collapsing of consecutive # console
## nodes. Don't bother doing this if there is only 1 new # command in the history
## or execution.  named.node.set <- FALSE start.node.created <- ''

## if (num.cmds > 0 && .ddg.is.init() && !inside.func && !called.from.ddg.eval) {
## .ddg.add.abstract.node('Start', node.name = node.name, env = environ)
## named.node.set <- TRUE start.node.created <- node.name }

## # Don't set .ddg.last.cmd.  We want it to have the value from # the last call.
## We set it at the end of this function: # .ddg.set('.ddg.last.cmd',
## .ddg.last.cmd)

## # Create an operation node for each command.  We can't use lapply # here
## because we need to process the commands in order and # lapply does not
## guarantee an order. Also decide which data nodes # and edges to create. Only
## create a data node for the last # write of a variable and only if that occurs
## after the last # possible writer. Create an edge for a data use as long as the
## # use happens before the first writer/possible writer or after # the last
## writer/possible writer. Lastly, if execute is set to # true, then execute each
## command immediately before attempting # to create the DDG nodes.

## # Only go through this if we have at least one command to parse.  if (num.cmds
## > 0) {

## # Find where all the variables are assigned for non-environ # files.  if
## (!execute) { vars.set <- .ddg.find.var.assignments(cmds) } else { vars.set <-
## .ddg.create.empty.vars.set() }

## # Loop over the commands as well as their string representations.  for (i in
## 1:length(cmds)) { cmd <- cmds[[i]]

## if (.ddg.debug.lib()) print(paste('.ddg.parse.commands: Processing',
## cmd@abbrev))

## # Process breakpoint. We stop if there is a breakpoint set on this line or we
## are single-stepping.  if (.ddg.is.sourced() & (cmd@is.breakpoint |
## .ddg.get('ddg.break')) & !.ddg.break.ignore()) { .ddg.process.breakpoint(cmd,
## inside.function=called.from.ddg.eval) }

## if (.ddg.enable.source() && grepl('^ddg.eval', cmd@text) &&
## .ddg.enable.console()) { if (is.null(.ddg.last.cmd)) { .ddg.last.cmd <- cmd } }

## # Get environment for output data node.  d.environ <- environ

## if ( .ddg.is.nonlocal.assign(cmd@parsed[[1]]) ) { d.environ <-
## .ddg.get.env(cmd@vars.set, for.caller=TRUE)
...
## || st.type == 'repeat' || st.type == '{')

## loop.statement <- (st.type == 'for' || st.type == 'while' || st.type ==
## 'repeat')

## # Specifies whether or not a procedure node should be created # for this
## command. Basically, if a ddg exists and the # command is not a DDG command or a
## control statement, it should # be created. Note that if control statements are
## annotated, # a procedure node is created for each statement inside a control #
## block, so there is no need to create additional nodes for the # control
## statement itself.

## create <- !cmd@isDdgFunc && .ddg.is.init() && .ddg.enable.console() &&
## !(control.statement && .ddg.loop.annotate() && ddg.max.loops() > 0) # create <-
## !cmd@isDdgFunc && .ddg.is.init() && .ddg.enable.console() start.finish.created
## <- FALSE cur.cmd.closed <- FALSE

## # If the command does not match one of the ignored patterns.  if
## (!any(sapply(ignore.patterns, function(pattern){grepl(pattern, cmd@text)}))) {

## # If sourcing, we want to execute the command.  if (execute) { # Print command.
## if (echo) { nd <- nchar(cmd@text) do.trunc <- nd > max.deparse.length cmd.show
## <- paste0(substr(cmd@text, 1L, if (do.trunc) max.deparse.length else nd),
## '\n') cat(cmd.show) }

## # If we will create a node, then before execution, set # this command as a
## possible abstraction node but only # if it's not a call that itself creates
## abstract nodes.  if (!cmd@isDdgFunc && cmd@text != 'next') {
## .ddg.set('.ddg.possible.last.cmd', cmd) .ddg.set ('.ddg.cur.cmd', cmd)

## # Remember the current statement on the stack so that we # will be able to
## create a corresponding Finish node later # if needed.  .ddg.cur.cmd.stack <-
## .ddg.get('.ddg.cur.cmd.stack')

## if (length(.ddg.cur.cmd.stack) == 0) { .ddg.cur.cmd.stack <- c(cmd, FALSE) }
## else { .ddg.cur.cmd.stack <- c(.ddg.get('.ddg.cur.cmd.stack'), cmd, FALSE) }
## .ddg.set('.ddg.cur.cmd.stack', .ddg.cur.cmd.stack) }

## else if (.ddg.is.procedure.cmd(cmd)) { .ddg.set('.ddg.possible.last.cmd', NULL)
## }

## # Need to get this number before evaluating the command so that # when we
## evaluate a dev.off call we know which device was closed
## .ddg.set('.ddg.dev.number', dev.cur())

## if (cmd@has.dev.off && !cmd@createsGraphics && is.null(.ddg.get
## ('possible.graphics.files.open'))) { dev.file.created <-
## .ddg.capture.current.graphics() } else { dev.file.created <- NULL }

## # Capture any warnings that occur when an expression is evaluated.  # Note that
## we cannot just use a tryCatch here because it behaves # slightly differently
## and we would lose the value that eval # returns.  withCallingHandlers returns
## the value.  # withCallingHandlers also re-throws the error after it is caught.
...
## { for (annot in cmd@annotated) { # Don't set return.value if we are calling a
## ddg function or we are executing an if-statement if (grepl('^ddg', annot) ||
## grepl('^.ddg', annot) || as.character(.ddg.get.statement.type(annot)) == 'if')
## { eval(annot, environ, NULL) } else { return.value <- eval(annot, environ,
## NULL) .ddg.set ('.ddg.last.R.value', return.value) } } }, warning =
## .ddg.set.warning , error = function(e) { # create procedure node for the
## error-causing operation .ddg.proc.node('Operation', cmd@abbrev, cmd@abbrev,
## env=environ, console=TRUE, cmd=cmd) .ddg.proc2proc()

## # create input edges by adding variables to set vars.set <-
## .ddg.add.to.vars.set(vars.set,cmd,i) if (.ddg.debug.lib())
## print(paste('.ddg.parse.commands: Adding', cmd@abbrev, 'information to
## vars.set, for an error')) .ddg.create.data.use.edges.for.console.cmd(vars.set,
## cmd, i, for.caller=FALSE)

## # create and link to an error node ddg.exception.out('error.msg', toString(e) ,
## cmd@abbrev) } )

## if (.ddg.debug.lib()) print (paste ('.ddg.parse.commands: Done evaluating ',
## cmd@annotated))


## if (!cmd@isDdgFunc && cmd@text != 'next') { # Need to get the stack again
## because it could have been # modified during the eval call.  .ddg.cur.cmd.stack
## <- .ddg.get('.ddg.cur.cmd.stack') stack.length <- length(.ddg.cur.cmd.stack)
## start.created <- .ddg.cur.cmd.stack[stack.length][[1]]

## # Create a finish node if a start node was created # start.created can have one
## of 3 values: 'TRUE', 'FALSE', # 'MATCHES_CALL'. Only create the finish node if
## TRUE.  if (start.created == 'TRUE') { .ddg.add.abstract.node('Finish', cmd,
## environ) start.finish.created <- TRUE .ddg.link.function.returns(cmd)

## # If the number of loop iterations exceeds max.loops, add # output data nodes
## containing final values to the finish node.  if (loop.statement &&
## .ddg.were.details.omitted()) { vars.set2 <- .ddg.add.to.vars.set(vars.set, cmd,
## i) .ddg.create.data.node.for.possible.writes(vars.set2, cmd, environ)
## .ddg.set.details.omitted(FALSE) } }

## # Remove the last command & start.created values pushed # onto the stack
## cur.cmd.closed <- (.ddg.cur.cmd.stack[stack.length] == 'MATCHES_CALL') if
## (stack.length == 2) { .ddg.set('.ddg.cur.cmd.stack', vector()) } else {
## .ddg.set('.ddg.cur.cmd.stack', .ddg.cur.cmd.stack[1:(stack.length-2)]) } }

## # Print evaluation.  if (print.eval) print(result) }

## # Figure out if we should create a procedure node for this # command. We don't
## create it if it matches a last command # (because that last command has now
## become a collapsible # node). Matching a last command means that the last
## command # is set, is not NULL, and is equal to the current command.

## last.proc.node.created <- if (.ddg.is.set
## ('.ddg.last.proc.node.created')).ddg.get('.ddg.last.proc.node.created') else ''

## create.procedure <- create && (!cur.cmd.closed || !named.node.set) &&
## !start.finish.created && !grepl('^ddg.source', cmd@text)

## # We want to create a procedure node for this command.  if (create.procedure) {

## # Create the procedure node.

## if (.ddg.debug.lib()) print(paste('.ddg.parse.commands: Adding operation node
## for', cmd@abbrev))

## .ddg.proc.node('Operation', cmd@abbrev, cmd@abbrev, env=environ, console=TRUE,
## cmd=cmd) .ddg.proc2proc()

## # If a warning occurred when cmd was evaluated, # attach a warning node if
## (.ddg.warning.occurred()) { .ddg.record.warning() } # Store information on the
## last procedure node in this # block.  last.proc.node <- cmd

## # We want to create the incoming data nodes (by updating # the vars.set).  if
## (execute) { # Add variables to set.  vars.set <-
## .ddg.add.to.vars.set(vars.set,cmd,i)

## if (.ddg.debug.lib()) print(paste('.ddg.parse.commands: Adding', cmd@abbrev,
...
## if (.ddg.debug.lib()) print(paste('.ddg.parse.commands: Adding input data nodes
## for', cmd@abbrev))

## .ddg.create.data.set.edges.for.cmd(vars.set, cmd, i, d.environ)

## if (.ddg.debug.lib()) print(paste('.ddg.parse.commands: Adding output data
## nodes for', cmd@abbrev))

## if (cmd@writesFile) .ddg.create.file.write.nodes.and.edges (cmd, environ) if
## (cmd@createsGraphics) .ddg.set.graphics.files (cmd, environ) if
## (cmd@updatesGraphics) .ddg.add.graphics.io (cmd)

## if (cmd@has.dev.off) { .ddg.capture.graphics(cmd)

## if (!is.null(dev.file.created)) { file.remove(dev.file.created) } } } # We
## wanted to create it but it matched a last command node.  else if (create &&
## execute) { .ddg.close.last.command.node(environ, initial=TRUE) if (execute) { #
## Add variables to set.  vars.set <- .ddg.add.to.vars.set(vars.set,cmd, i) if
## (.ddg.debug.lib()) print(paste('.ddg.parse.commands: Adding', cmd@abbrev,
## 'information to vars.set')) .ddg.create.data.set.edges.for.cmd(vars.set, cmd,
## i, environ) } }
...
## # Update so we don't set these again.  vars.set$possible.last.writer <-
## vars.set$last.writer } } }

## # Create a data node for each variable that might have been set in # something
## other than a simple assignment, with an edge from the # last node in the
## console block or source .  if (!execute) {
## .ddg.create.data.node.for.possible.writes(vars.set, last.proc.node,
## env=environ) } }

## # Close any node left open during execution.  if (execute && !inside.func)
## .ddg.close.last.command.node(environ, initial=TRUE)

## # Close the console block if we processed anything and the DDG # is initialized
## (also, save).  # if (.ddg.is.init() && named.node.set && !inside.func) {
## .ddg.add.abstract.node('Finish', node.name = node.name, env=environ) }

## # Open up a new collapsible node in case we need to parse # further later.  if
## (!execute) {

## .ddg.set('.ddg.possible.last.cmd', .ddg.last.cmd) .ddg.set('.ddg.last.cmd',
## .ddg.last.cmd) .ddg.open.new.command.node(environ) }

## # Write time stamp to history.  if (.ddg.is.init() && !.ddg.is.sourced())
## .ddg.write.timestamp.to.history()

## return.value <- .ddg.get ('.ddg.last.R.value') return(return.value) }


# Returns TRUE if the value of the given variable name is a data frame containing
# at least one factor. Returns FALSE otherwise.  var - the variable name
